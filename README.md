...
| `FLASHINFER_AITER_ALLOW_UNVALIDATED` | `0` | Set to `1` to use AITER prefill with an `amd-aiter` newer than the last validated release. FlashInfer links AITER's prefill kernels by mangled symbol and passes a vendored argument struct, so an unvalidated build can silently return wrong results; by default `backend="aiter"` raises and `backend="auto"` falls back to `fa2`. Only set this if you have checked the numerics yourself. |

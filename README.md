...
| `FLASHINFER_HIP_FUSED_CASCADE` | `0` | Fold each cascade level's merge into its prefill kernel instead of running the level and then `merge_state_in_place`. Opt-in performance variant; both paths are covered by the suite. Read once at import, so set it before importing `flashinfer`. |

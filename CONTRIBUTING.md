...
**The last measured run is committed** at [`docs/rocm/coverage-gfx942.json`](docs/rocm/coverage-gfx942.json), so a change that drops coverage shows up as a reviewable diff rather than going unnoticed until someone spends 75 minutes re-running the suite. Refresh it in the same commit as any change that moves the number, and before each `+amd.N` tag, with `--json-out docs/rocm/coverage-gfx942.json`. There is no automated gate; this is the ratchet.

**Say what the AITER lib cache was.** `jit/aiter_source.py`'s build path executes only when `ensure_aiter_lib` actually builds, so a cold `~/.cache/flashinfer` measures **32 lines higher** than a warm one on identical code — about 0.9 points. Measured 2026-08-28 on gfx942: 3146/3502 cold against 3126/3502 warm. Neither is wrong, but two numbers only compare if the cache policy matched. The committed payload above is a **warm-cache** run; regenerate it warm unless you also update this sentence.

...
#
# Re-measured on amd-aiter 0.1.21.post2, 2026-09-14, over non-square
# qo_len x kv_len x cap at head_dim=128 causal, vs an fp32 reference, with this
# guard disabled so the kernel runs. Unchanged: gfx950 wrong on all 12 capped
# rows (9 at 1.19-4.11 absolute, 3 returning NaN) where fa2 is 0.0001-0.0011 on
# the same shapes; gfx942 clean on all 12. cap=0 is clean on both.
...
# a +/-1.3% noise floor:
# Single prefill is batch=1, so only the batch-1 half of the sweep is reachable
# here; those 12 cells are what the threshold rests on.
#   gfx950  climbs with length above seqlen 1024; at/above 2048 the batch-1
#           cells give a 1.14x geomean, worst 0.98 at 16 heads / seqlen 2048
#   gfx942  non-monotonic and undamped, and the batch-1 gate region is a net
#           loss (0.98x geomean, 8 of 12 regressing) -- asm stays unreachable
...
CAPABILITIES: Tuple[Capability, ...] = (
    ...
    Capability(
        ...
        note="Paged and ragged, with sliding window. fp8 query/KV on the paged route (bf16 out, per-tensor descales, no LSE). Page sizes 1/16/1024 are native; fp16/bf16 are routed there only at 1024 and flat-gather below it, which measured faster.",
        fallback="fa2",
    ),

...
- **AITER backend constraints, accurately:**
  ...
  - **"Native" page sizes** (no flat-gather): `{128, 256, 1024}` — see `_aiter_native_page_sizes()` in [`rocm/prefill.py`](../../../flashinfer/rocm/prefill.py). The `{16, 1024}` arm there is unreachable now that `flashinfer.rocm.aiter_utils.AITER_MIN_VERSION` floors AITER at 0.1.21. It is only a hint: `plan()` probes with `_aiter_native_paging_available()` and degrades to flat-gather if the installed AITER cannot serve the config. **Non-native page sizes are NOT rejected** — they flat-gather. So the "{1, 16, 1024}" guidance from older docs is wrong. **Measured on amd-aiter 0.1.21.post2, all three advertised sizes fail the probe** (`no matching kernel found`) on gfx942 *and* gfx950, so every paged prefill flat-gathers and `mha_batch_prefill` is unreachable — benchmark the paged path as flat-gather until that is fixed.
